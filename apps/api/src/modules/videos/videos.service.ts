...
export const videosService = {
  async list(input: { page: number; limit: number; status?: VideoStatus; uploadedBy?: string }) {
  ...
  async create(input: {
    ...
    return { ...created, uploadedBy: await usersRepository.findById(input.actorId) }
  },
  ...
  async update(input: { videoId: string; actorId: string; title?: string; description?: string }) {
    ...
    return {
      type: 'ok' as const,
      video: { ...updated, uploadedBy: await usersRepository.findById(updated.uploadedById) },
    }
  },
